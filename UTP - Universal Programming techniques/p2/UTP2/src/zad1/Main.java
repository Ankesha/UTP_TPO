...
public class Main {
  ...
  public List<Integer> test1(List<Integer> src) {
    Selector<Integer> sel = new Selector<Integer>() {
      @Override
      public boolean select(Integer jakisTyp) {
        if (jakisTyp < 10) {
          return  true;
        } else return false;
      }
    ...
    Mapper<Integer, Integer> map = new Mapper<Integer, Integer>() {
      @Override
      public Integer map(Integer jakisInnyTyp) {
        jakisInnyTyp += 10;
        return jakisInnyTyp;
      }
  ...
  public List<Integer> test2(List<String> src) {
    Selector<String> sel = new Selector<String>() {
      @Override
      public boolean select(String jakisTyp) {
        if (jakisTyp.length()>3) {
          return true;
        } else return false;
      }
    }; /*<-- definicja selektora; bez lambda-wyrażeń; nazwa zmiennej sel */
    Mapper<String, Integer> map = new Mapper<String, Integer>() {
      @Override
      public Integer map(String jakisInnyTyp) {
        Integer res = jakisInnyTyp.length() + 10;
        return res;
      }
    };
      /*<-- definicja mappera; bez lambda-wyrażeń; nazwa zmiennej map */
